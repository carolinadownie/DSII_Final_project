...
hospital_data_regions_numeric <- hospital_data_regions %>% mutate(region_num = ifelse(grepl("New England", region), "1", ifelse(grepl("Middle Atlantic", region), "2", ifelse(grepl("East North Central", region), "3", ifelse(grepl("West North Central", region), "4", ifelse(grepl("South Atlantic", region), "5", ifelse(grepl("East South Central",region), "6", ifelse(grepl("West South Central", region), "7", ifelse(grepl("Mountain", region), "8",ifelse(grepl("Pacific", region), "9", ifelse(grepl("U.S. Territories", region), "10", "0"))))))))))) %>% 
  ...
  mutate(region_num = as.factor(region_num), hospital_type_num = as.factor(hospital_type_num), hospital_ownership_num = as.factor(hospital_ownership_num), patient_experience_num = as.factor(patient_experience_num)) %>% mutate(hospital_overall_rating = as.factor(hospital_overall_rating))
...
hospital_data_regions_numeric %>% mutate(hospital_overall_rating = as.factor(hospital_overall_rating)) %>% mutate(hospital_ownership_num = as.factor(hospital_ownership_num)) %>% ggplot(aes(x = region_num, y = hospital_ownership_num, color = hospital_overall_rating)) + geom_point() 
...
#Plotting clusters?
plot(km_hospitals_train, col = (km_hospitals$cluster + 1))
...
#Creating regression tree
hospital_tree <- tree(hospital_overall_rating ~ region_num + hospital_type_num + hospital_ownership_num + patient_experience_num, data = hospitals_train)

plot(hospital_tree)
text(hospital_tree, pretty = 0)

```

Cross-validation
```{r cross-validation of the tree}
cv_hospital_tree <- cv.tree(hospital_tree)

plot(cv_hospital_tree)
text(cv_hospital_tree)

prune_hospital_train <- prune.tree(hospital_tree, best = 2)
plot(prune_hospital_train)
text(prune_hospital_train, pretty = 0)
...
Test set tree
```{r}
hospital_test_tree <- predict(hospital_tree, newdata = hospitals_test)

test_MSE <- mean((hospital_test_tree - hospitals_test$hospital_overall_rating)^2)
...
#Version 2--no patient experience

```{r}
library(tree)

#Creating regression tree
hospital_tree2 <- tree(hospital_overall_rating ~ region_num + hospital_type_num + hospital_ownership_num , data = hospitals_train)

plot(hospital_tree2)
text(hospital_tree2, pretty = 0)
